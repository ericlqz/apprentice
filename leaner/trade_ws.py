...
def sub_topic(ws):
    ...
    for ticker in tickers:
        ws.send(json.dumps({"op": "sub", "topic": 'trade:' + ticker}))
...
def save_data(data):
    ...
    message_json["amount"]=float(message_json["amount"])
    message_json["price"]=float(message_json["price"])
    message_json["volume"]=float(message_json["volume"])
    message_json["tpp"]=int(message_json["tpp"])

    # del message_json["timestamp"]

    res = es.index(index="trade_hoo", body=message_json)
    # print(res)
